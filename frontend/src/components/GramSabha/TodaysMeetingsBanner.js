...
import {
  ...
  addAttendance,
  fetchTodaysMeetings,
  startJioMeetRecroding,
  stopJioMeetRecroding,
  generateJWTToken,
  getJioMeetRecroding,
} from "../../api/gram-sabha";
...
const TodaysMeetingsBanner = ({ panchayatId, user }) => {
  ...
  useEffect(() => {
    const unsubscribe = EventManager.onEvent((eventInfo) => {
      ...
      switch (type) {
        case "PEER_JOINED":
          console.log("A new peer has joined:", data.remotePeer);
          break;
        case "PEER_UPDATED":
          console.log("Peer updated:", data.remotePeer);
          break;
        case "PEER_LEFT":
          console.log("Peer left:", data.remotePeer);
          break;
        case "MEETING_JOINED":
          console.log("Joined fully>>>>>>>>>>>>>>>>>>>>", data);
          break;
        default:
          break;
      }
  ...
  //     hostToken
  //   ) => {
  //     // If meetingLink exists, navigate to it, otherwise show meeting details
  //     if (meetingLink) {
  //       window.open(meetingLink, "_blank");
  //       await sleep(5000);

  //       // await handleRecordMeeting(meetingId, roomPIN, hostToken);
  //     } else {
  ...
  const handleListRecording = async (meetingId, roomPIN, historyId) => {
    try {
      // const response = await getJioMeetRecroding(meetingId, roomPIN, historyId);
      //setRecordings(response.callRecordings);
      // You can show a success toast or message here
    } catch (error) {
      console.error(
        "Recording can only be started when user has joined the meeting",
        error
      );
      alert(`Recording can only be started when user has joined the meeting`);
    }
  };

  const handleStopRecording = async (meetingId, roomPIN) => {
  ...
  const handleRecordMeeting = async (meetingId, roomPIN, hostToken) => {
    setIsStarting(true);
    if (!meetingId || !roomPIN) {
      console.warn("Meeting ID and Room PIN are required to start recording.");
      return;
    }

    const userRole = "host"; // Change this based on user permissions
    const config = { userRole, token: hostToken };

    try {
      const meetingOptions = {
        meetingId: meetingId,
        meetingPin: roomPIN,
        userDisplayName: "Host User", // Optional display name
        config,
      };

      const joinResponse = await jmClient.joinMeeting(meetingOptions);
      setHistoryId(joinResponse);
      //console.log(joinResponse, "Joined meeting with this user");
      // Wait a few seconds for meeting to fully initialize
      ...
      console.error("Failed to join or start recording", error);
      alert("Make sure you're host and joined the meeting");
    }
    setIsStarting(false);
  };
      ...
      >
        ...
        <CardContent sx={{ px: 3, py: 2 }}>
          ...
          <Box display="flex" justifyContent="flex-end" gap={2} sx={{ mt: 1 }}>
            ...
            {/* {historyId !== null && isRecording === false && (
              <Button
                variant="contained"
                color="success"
                onClick={() =>
