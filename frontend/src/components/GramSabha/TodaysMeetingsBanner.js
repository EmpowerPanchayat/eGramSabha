...
const TodaysMeetingsBanner = ({ panchayatId, user }) => {
    ...
    const loadFaceModels = async () => {
        ...
        try {
            ...
            setAttendanceMessage({ type: 'error', text: 'Error loading facial recognition models. Please try again.' });
            return false;
        } finally {
    ...
    const loadAttendanceStats = async (meetingId) => {
        try {
            ...
            const response = await fetch(`${API_URL}/gram-sabha/${meetingId}/attendance-stats`, {
                headers: {
    ...
    const handleStartRecording = (meetingId, meetingLink) => {
    ...
    const startCamera = async () => {
        try {
            ...
            if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
                setAttendanceMessage({ type: 'error', text: 'Camera access denied. Please allow camera access.' });
            } else {
                setAttendanceMessage({ type: 'error', text: `Error accessing camera: ${error.message}` });
            }
        }
    };
    ...
    const handleSubmitAttendance = async () => {
        if (!voterIdLastFour || voterIdLastFour.length !== 4) {
            setAttendanceMessage({ type: 'error', text: 'Please enter the last 4 digits of the Voter ID.' });
            return;
        }

        if (!faceDetected) {
            setAttendanceMessage({ type: 'error', text: 'No face detected. Please position your face in front of the camera.' });
            return;
        }
        ...
        try {
            ...
            if (!detections) {
                setAttendanceMessage({ type: 'error', text: 'Face not recognized clearly. Please try again.' });
            ...
            const response = await fetch(`${API_URL}/gram-sabha/${todaysMeetings[0]._id}/mark-attendance`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    'Content-Type': 'application/json'
            ...
            if (attendanceStats.present + 1 >= attendanceStats.quorum && !attendanceStats.quorumMet) {
                ...
                <CardContent sx={{ px: 3, py: 2 }}>
                    ...
                    >
                        ...
                        <Button
                            variant="contained"
                            color="success"
                            onClick={() => handleStartRecording(meeting._id, meeting.meetingLink)}
                    ...
                    {attendanceStats && (
                        <Box sx={{ mb: 4, mt: 2 }}>
                            ...
                            <Box sx={{ mt: 2 }}>
                                ...
                                <LinearProgress
                                    variant="determinate"
                                    value={attendanceStats.quorum > 0 ? (attendanceStats.present / attendanceStats.quorum) * 100 : 0}
